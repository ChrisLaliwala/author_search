...
@click.command(help='author keywords. Pass up to 3 separated by spaces. Ex: author_search three keywords max')
...
def main(keywords):
    ...
    for count, author in enumerate(sorted_hindex_dict):
        if count < 5:
            print(f'{author_data_dict[author]['display_name']}, {author}, h-index: {sorted_hindex_dict[author]}')
